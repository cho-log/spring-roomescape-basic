...
function render(data) {
  ...
  data.forEach(item => {
    ...
    row.insertCell(3).textContent = item.schedule.time;
...
function fetchSchedules() {
  requestRead(SCHEDULE_API_ENDPOINT)
      .then(data => {
        schedulesOptions.push(...data);
      })
...
function addInputRow() {
  ...
  const scheduleDropdown = createSelect(schedulesOptions, "시간 선택", 'schedule-select', 'time');

  const cellFieldsToCreate = ['', nameInput, dateInput, scheduleDropdown];
...
function saveRow(event) {
  ...
  const scheduleSelect = row.querySelector('select');
